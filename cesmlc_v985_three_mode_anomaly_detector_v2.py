...
python "C:\Temp\cesmlc_v985_three_mode_anomaly_detector.py" `
  ...
  --out-dir "C:\Temp\v985_compare_outputs" `
  ...
  --history-cache-dir "C:\Temp\cesmlc_v985_window_gate_cache" `
...
7. Exactly four final clusters are possible:
      ...
      RARE_CHANGES            recurrent unusual behavior; a current/repeated
                              ...
                              in Mode 2, the fallback for an IF-admitted ECGI
                              with no BF/HF/MULTI structural signature
8. Defaults, fallback values, location-shift magnitude, invalid-range rules,
   ...
   an incident.
9. In Mode 1, scores only rank rule-qualified incidents. Mode 2 deliberately
   uses ``--if-dominant-percentile`` as its first and only admission gate; after
   admission, signature inspection maps the label and cannot delete the ECGI.
...
def _build_mode2_view(
    ...
    result["ONE_DAY_RARE_GUARD_FLAG"] = 0
...
label and cannot remove the ECGI.
